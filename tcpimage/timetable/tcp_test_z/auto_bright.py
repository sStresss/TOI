...
class AutoBright:
    ...
    def set_auto_bright(self):
        ...
        print(self.list_bright, '00000000000')

        len_arr = len(self.list_bright)
...
if __name__ == "__main__":
    auto_bright = True  # смена режима работы яркости
    ...
    for item in a_str.split(','):
        str_table = item[1:-1]
        minute = str_table.split('|')[0].replace("'", '')
        hours = str_table.split('|')[1].replace("'", '')
        bright = str_table.split('|')[2].replace("'", '')

        list_item = ['true', '0 ' + minute + ' ' + hours, bright]
